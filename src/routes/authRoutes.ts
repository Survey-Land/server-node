...
// Admin only routes
router.post("/admin", authenticateJWT, validate(adminCreateSchema),  authController.createAdmin);
router.delete("/users/:id", authenticateJWT,  authController.deleteUser);
router.patch("/users/:id/role", authenticateJWT, validate(updateRoleSchema),  authController.updateUserRole);
