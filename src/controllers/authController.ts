...
export class AuthController {
  ...
  register = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      res.status(201).json({ message: i18n.__('User registered successfully'), token,  user: {
        id: user.id,
        email: user.email,
        name: user.name,
        role: user.role,
  ...
  login = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const token = generateToken({ id: user.id, email: user.email, role: user.role })
      res.json({ message: i18n.__('Logged in successfully'), token,  user: {
        id: user.id,
        email: user.email,
        name: user.name,
        role: user.role,
  ...
  googleCallback = (req: Request, res: Response, next: NextFunction) => {
    passport.authenticate('google', { session: false }, (err, user) => {
      if (err || !user) return res.status(400).json({ message: i18n.__('Authentication failed') })
      const token = generateToken({ id: user.id, email: user.email, role: user.role })
      res.json({ message: i18n.__('Logged in successfully with Google'), token,  user: {
        id: user.id,
        email: user.email,
        name: user.name,
        role: user.role, 
