...
export class SurveyController {
    ...
    public async getSurvey(req: Request, res: Response, next: NextFunction) {
        try {
            const lang = this.setLocale(req);
            const survey = await this.surveyService.getSurvey(
                req.params.id,
                lang
            );
    ...
    public async delete(req: Request, res: Response, next: NextFunction) {
        try {
            const lang = this.setLocale(req);
            const survey = await this.surveyService.deleteSurvey(
                req.params.id,
                lang
            );
    ...
    public async getSurveyByLink(
        ...
        try {
            const lang = this.setLocale(req);
            const survey = await this.surveyService.getSurveyByLink(
                req.params.link
            );
            res.status(200).json(survey);
        } catch (error) {
