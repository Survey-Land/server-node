...
class AuthService {
  async findAll(query, lang) {
    ...
    try {
      ...
      if (include) include = (0, query_parser_1.parseInclude)(include);
      if (orderBy) orderBy = (0, query_parser_1.parseInclude)(orderBy);
  ...
  async login(email, password, lang) {
    ...
    if (!user || !user.password)
      throw new custom_error_1.CustomError(
        i18n_1.default.__("Incorrect email or password"),
        400
      );
  ...
  async registerInit({ email, password, name }, lang) {
    ...
    try {
      await (0, emailService_1.sendOtpEmail)(
        email,
        i18n_1.default.__("Verify your email"),
        otpPlain,
        i18n_1.default.__("valid for %s minutes", "5")
      );
  ...
  async verifyOtp({ email, otp }, lang) {
    ...
    return prisma_1.default.$transaction(async (tx) => {
      ...
      const match = await (0, otp_generation_1.compareOtp)(
        otp,
        otpRecord.otpHash
      );
      if (!match) throw new Error(i18n_1.default.__("Invalid OTP"));
  ...
  async getProfile(id) {
    ...
    if (!user)
      throw new custom_error_1.CustomError(
        i18n_1.default.__("User not found"),
        404
      );
    return user;
  }
